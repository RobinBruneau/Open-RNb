"""Tests for the RNb dataset loading pipeline.
...
FIXTURE_DIR = os.path.join(ROOT, "tests", "data", "golden_snail_rnb_mini")
N_VIEWS = 3
IMG_W, IMG_H = 32, 28
...
class TestLoadKRtFromP:
    ...
    def test_returns_intrinsics_and_pose(self):
        from datasets.rnb import load_K_Rt_from_P
...
class TestRNbDataset:
    @pytest.fixture(autouse=True)
    def _patch_rank(self, monkeypatch):
        monkeypatch.setattr("datasets.rnb.get_rank", lambda: "cpu")
    ...
    def test_tensor_shapes(self):
        from datasets.rnb import RNbDataset
        ds = RNbDataset(self._make_config(), "train")
    ...
    def test_downscale(self):
        from datasets.rnb import RNbDataset
        ds = RNbDataset(self._make_config(img_downscale=2), "train")
        assert ds.w == IMG_W // 2
        assert ds.h == IMG_H // 2
    ...
    def test_test_split_combinations(self):
        from datasets.rnb import RNbDataset
        ds = RNbDataset(self._make_config(), "test")
    ...
    def test_getitem_returns_index(self):
        from datasets.rnb import RNbDataset
        ds = RNbDataset(self._make_config(), "val")
        item = ds[1]
...
class TestRNbDataModule:
    ...
    def test_setup_idempotent(self):
        from datasets.rnb import RNbDataModule
        dm = RNbDataModule(self._make_config())
        dm.setup("fit")
    ...
    def test_registry(self):
        import datasets
        assert "rnb" in datasets.datasets
